...
async function seed() {
  ...
  const essays = [
    {
      ...
      topic: "法学部志望理由書",
      ocrText:
        "私が京都大学法学部を志望する理由は、法と社会の接点に対する深い関心にあります。",
      imageUrl: "",
      status: "reviewed",
    ...
    },
    {
      ...
      topic: "法学部志望理由書（改訂版）",
      ocrText:
        "私が京都大学法学部を志望する理由は、高校2年次の模擬裁判経験を通じて法と社会の接点に強い関心を持ったからです。",
      imageUrl: "",
      status: "reviewed",
      ...
      feedback: {
        overall:
          "前回より大幅に改善されました。構成が明確になり、具体例も充実しています。",
        goodPoints: [
          "模擬裁判の経験が説得力を持つ",
          "構成の明確さ",
          "AP連動の改善",
    ...
    },
    {
      ...
      ocrText:
        "同志社大学法学部を志望する理由は、国際法分野での充実した教育環境にあります。",
      imageUrl: "",
      status: "reviewed",
      ...
      feedback: {
        overall:
          "非常によくまとまった志望理由書です。AP合致度も高く、説得力があります。",
        goodPoints: [
          "国際法への関心が具体的",
          "良知の精神との関連が自然",
          "構成力の向上",
  ...
  const interviews = [
    {
      ...
      conversationSummary:
        "志望動機は明確だが、具体的なエピソードが不足。質問への応答に間が多い。",
    },
    {
      ...
      conversationSummary:
        "前回より大幅に改善。具体例を交えた回答ができるようになった。表現力も向上。",
    },
  ];
  for (const interview of interviews) {
    const { id, ...data } = interview;
    await setIfNotExists(
      db.doc(`interviews/${id}`),
      data,
      `interview: ${data.universityContext.universityName}`
    );
  }
  ...
  for (const act of activities) {
    const { id, ...data } = act;
    await setIfNotExists(
      db.doc(`users/${DEV_USER_ID}/activities/${id}`),
      data,
      `activity: ${data.title}`
    );
  }
  ...
  for (const doc of documents) {
    const { id, ...data } = doc;
    await setIfNotExists(
      db.doc(`users/${DEV_USER_ID}/documents/${id}`),
      data,
      `document: ${data.title}`
    );
  }
  ...
  console.log("\n--- Sessions ---");
  await setIfNotExists(
    db.doc("sessions/session_dev_001"),
    {
