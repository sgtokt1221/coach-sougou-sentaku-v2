...
import {
  createVoiceSession,
  type VoiceProvider,
} from "@/lib/interview/realtime/voice-session-factory";
import {
  GdOrchestrator,
  type GdOrchestratorTokens,
} from "@/lib/interview/realtime/gd-orchestrator";
...
interface UseRealtimeInterviewOptions {
  ...
  /**
   * 面接セッションID。サーバーが開始時に保存した設定（口頭試問の分野など）を
   * 引くために渡す。渡さなくても接続はできる。
   */
  sessionId?: string;
  universityId?: string;
  facultyId?: string;
...
export function useRealtimeInterview(options: UseRealtimeInterviewOptions) {
   ...
   * 常に null (未使用)。
   */
  const [currentSpeaker, setCurrentSpeaker] = useState<ActiveSpeaker | null>(
    null
  );
  /**
  ...
  const appendMessage = useCallback(
    (m: InterviewMessage, audioWavBase64?: string) => {
  ...
  const start = useCallback(
    async (startOpts?: {
      priorMessages?: InterviewMessage[];
      ...
      const effectiveProvider: VoiceProvider =
        optsRef.current.provider ?? "gemini";
      const tokenEndpoint =
      ...
      try {
        ...
        if (tokenData.rateLimited) {
          ...
          return {
            ...
            nextAvailableAt: tokenData.nextAvailableAt,
          };
        }
        if (!res.ok || !tokenData.tokens || tokenData.tokens.length === 0) {
          const debugInfo = (tokenData as { debug?: unknown }).debug;
          console.warn(
            "[useRealtimeInterview] token fetch rejected",
            res.status,
            tokenData
          );
          const detail = debugInfo
            ? JSON.stringify(debugInfo).slice(0, 300)
            : (tokenData.error ?? `HTTP ${res.status}`);
      ...
      try {
        const model =
          tokenData.model ?? "gpt-4o-mini-realtime-preview-2024-12-17";

        if (optsRef.current.mode === "group_discussion") {
          // 討論テーマを UI 表示用に保持 (司会が開幕で読み上げるのと同じ値)
          const themeVal = (tokenData as { theme?: string | null }).theme;
          setGdTheme(
            typeof themeVal === "string" && themeVal.trim() ? themeVal : null
          );

          // GD: 3 並列セッションを GdOrchestrator で束ねる
          const gdTokens: GdOrchestratorTokens[] = tokenData.tokens.map(
            (t) => ({
          ...
          const orch = new GdOrchestrator({
            ...
            contextMessage:
              typeof (tokenData as { contextMessage?: string })
        ...
        const session = createVoiceSession({
          ...
          onUserTranscript: (text, audioWavBase64) => {
            ...
            if (isPromptEcho(text, transcriptionPrompt)) {
              console.warn(
                "[useRealtimeInterview] dropping echoed transcription-prompt:",
                text.slice(0, 60)
              );
              return;
            }
            ...
            if (isLikelyNoise(text)) {
              console.warn(
                "[useRealtimeInterview] dropping noise-like transcript:",
            ...
            if (isAiRespondingRef.current) {
              console.warn(
                "[useRealtimeInterview] dropping transcript during AI turn:",
                JSON.stringify(text).slice(0, 40)
              );
              return;
            }
          ...
          onOutputLevelUnsupported: () => {
            ...
            console.warn(
              "[useRealtimeInterview] output audio level unsupported; using fixed-delay turn switch"
            );
            // 既に復帰待ちなら固定遅延で復帰させる
        ...
        const prior =
          startOpts?.priorMessages?.filter((m) => m.content?.trim()) ?? [];
        if (prior.length > 0) {
