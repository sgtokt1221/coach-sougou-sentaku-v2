 ...
 * 引用。
 *
 * 元が後から消えても引用は残したいので、本文を写して持つ（参照だけ持つと
 * 表示できなくなる）。partial は選択範囲だけを引いたか。
 *
 * チャットの発言を引くとき以外にも使う。答案や志望理由書の一部に付けた
 * 範囲コメントでは、引用元がチャットの発言ではないので messageId は無い。
 */
export interface ChatQuote {
  /** 引用元のチャット発言ID。答案・書類の範囲引用では付かない */
  messageId?: string;
  authorName: string;
