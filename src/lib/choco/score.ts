...
/**
 * ちょこ添削で採点できない2軸（回答力・議論の成熟度）を、採点できた3軸の平均から
 * 何点低く見積もるか。
 *
 * ちょこ添削は完成した小論文の1段落だけを書く練習なので、回答力と議論の成熟度は
 * 構造的に測れない（設問も論の骨格も与えられており、答案全体が設問に答えて
 * いるかを見る余地がない）。合計を0-50へ換算するには
 * この2軸を何らかの形で置くしかないため、本添削の実データから差を取った。
 *
 * 本番の小論文添削20件（成熟度あり）の実測:
 *   構成/論理/表現の平均 4.97 / 旧・独自性＋成熟度の平均 4.08 → 差 0.89
 *   （独自性は v23 で回答力に置き換わった。配点の合計(15)は変わらないので
 *     この換算値はそのまま使う。回答力の実データが溜まったら測り直すこと）
 *
...
export function computeChocoTotal(
  ...
  const weighted =
    ...
    estimated *
      (ESSAY_SCORE_WEIGHTS.responsiveness +
        ESSAY_SCORE_WEIGHTS.reasoningMaturity);
