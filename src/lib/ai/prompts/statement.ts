...
## 構成指針（各段落の分量は目標文字数 ${target}字 に対する比率。合計で ${target}字±10%に必ず収める）
【導入部】 (全体の約20%)
- 自己の価値観や原体験から始める
- 志望分野への関心のきっかけを示す
...
- 段落と段落を自然につなぐ。各段落は前段落の内容を受けてから次の話題に移り、唐突な切り替えを避ける。段落冒頭でつなぎ表現（例:「こうした経験から」「この関心は」「だからこそ」等）を過度でない範囲で用い、全体が一つの物語として滑らかに流れるようにする（箇条書きの寄せ集めにしない）
- 文体は「である調」で統一する
