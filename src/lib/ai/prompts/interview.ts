...
- 面接として成立していない（質問に対する実質的な回答がない）場合、各項目0〜2点にしてください
- 会話のターン数が少ない場合、評価材料が不足しているため高得点はつけないでください
- 7点以上は「明確に優れている」場合のみ、9〜10点は「模範的」な場合のみ付与してください

## 点数の目安（各項目10点満点）
- **0〜2点**: 面接として不成立、または評価材料がほぼない
- **3〜4点**: 最低限の回答はあるが、内容が薄い・曖昧
- **5〜6点**: 平均的。基本的な受け答えはできているが改善の余地が大きい
- **7〜8点**: 良好。具体的で論理的な回答ができている
- **9〜10点**: 非常に優れている。模範的な回答
...
\`\`\`json
{
  ...
  "feedback": {
    ...
    "personalizedAdvice": ["<自己分析データを踏まえた個別アドバイス1>", "<個別アドバイス2>", ...],
    "repeatedIssues": [
      {
        "area": "<弱点領域>",
        "count": <繰り返し回数>,
