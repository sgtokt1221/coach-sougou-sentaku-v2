...
- 小論文用の類題 (\`essayQuestions\`) を **必ず 3 件**
- 面接用の類題 (\`interviewQuestions\`) を **必ず 3 件**
- 合計 **6 件は必須**。空配列で返すことは絶対に禁止します。

### データが不足している場合の方針
弱点リストが空、または過去テーマの記録が乏しい場合でも、**総合型選抜で頻出する一般的なテーマ** から選んで必ず 6 件提案してください。例:
- 小論文: 探究学習の意義 / SNS と情報リテラシー / 地域社会の課題 / 多様性と共生 / 環境問題 / グローバル化への向き合い方
- 面接: 志望理由 / 高校生活で頑張ったこと / 強みと弱み / 将来のキャリア / 大学で学びたいこと / 印象に残る経験

その場合 \`relatedWeakness\` には「総合型選抜の基礎力強化」「定番テーマでの表現練習」など妥当な理由を書いてください。
