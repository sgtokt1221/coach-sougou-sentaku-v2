...
## 文章レベルの指摘（languageCorrections）
以下の観点で、文章中の具体的な問題箇所を全て指摘してください：
- **typo**: 誤字脱字、変換ミス、送り仮名の誤り
- **grammar**: 主語と述語の不一致、助詞の誤用、文法的な誤り
- **connector**: 接続語の不適切な使用（「しかし」の連続、因果関係のない「したがって」、逆接が不要な「だが」等）
- **expression**: 口語的すぎる表現（「やっぱり」「すごく」等）、小論文に不適切な表現、より適切な言い換え
- **redundancy**: 冗長な表現（「〜ということ」の多用、同じ意味の繰り返し等）の簡潔化
