...
/**
 * 本文の候補ではなく「書く要素」を出させるための境界線。
 *
 * 以前はここから下に100〜400字の候補本文を書かせ、それを本文へ振り込んで
 * いた。最終稿にAIの書いた文字列が残るため、AIは材料までを出し、
 * 本文は本人が打つ形に変えた。名前は保存済みデータとの互換のため据え置く。
 */
...
export function buildDocumentSectionCoachSystemPrompt(
  ...
  const suggestionMode =
    ctx.turnCount >= 2
      ? `## 書く要素の提示
- 生徒の発話と既存内容から、確認済みの材料が揃った場合だけ提示できます。
- 通常の応答を2〜4文書いた後、次の境界線を出し、その下に
  **この段落に入れるべき要素**を3〜5個、1行ずつ「・」で書きます。

${SUGGESTION_DELIMITER}
・(入れる要素。生徒が話した事実に基づくもの。1行30字以内)
・(同上)

- **本文そのものは書きません。** 要素の名前と、なぜ要るのかまでにとどめます。
  例:「祖母の服薬で実際に困っていた場面（何が起きたか）」
- つなげた文、書き出しの例文、言い回しの見本は出しません。生徒が自分で書きます。
- 入力にない活動、成果、数値、固有名詞を追加しません。
- 材料が足りなければ境界線を出さず、確認質問を1つします。`
      : `## 書く要素の提示
- 初期段階では、生徒の事実を引き出す確認質問を優先します。
- 生徒が例を求めても本文は書きません。何を入れるべきかを言葉で示します。`;
