...
import { useMarkViewedOnOpen, type DetailDialogProps } from "./shared";
...
/**
 * 出願書類1件の詳細（承認/差し戻しのレビュー用モーダルを含む）。
 * 開いたときに詳細 API から取る（一覧には本文・版の履歴が無い）。
 */
export function DocumentDetailDialog({
  ...
  /** 一覧（AIスコア・レビュー状態の列）を最新にする */
  const mutate = () => mutateSWR(`/api/admin/students/${studentId}/documents`);
  useMarkViewedOnOpen("document", id, studentId);
    ...
    (async () => {
      ...
      try {
        const res = await authFetch(
          `/api/admin/students/${studentId}/documents/${id}`
        );
  ...
  }, [id, studentId, reloadKey]);

  // 閉じるアニメーションの間は直前の中身を残す
  const detailLoading = id !== null && result?.id !== id;
  const detailDoc = detailLoading ? null : (result?.data ?? null);
  const setDetailDoc = (doc: DocumentDetail) =>
    setResult({ id: doc.id, data: doc });
