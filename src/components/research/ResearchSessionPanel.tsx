...
interface ResearchSessionPanelProps {
  /** 既存セッション画面などに埋め込む場合 true。未受講の生徒には何も描画しない。 */
  embedded?: boolean;
}

/**
 * 自己探究授業「教える」セッションの本体パネル。
 * 受講登録＋保護者同意(granted)が揃った生徒のみ録音・AI講評を利用できる（同意ゲート）。
 * 単体ページ /student/research/session と 面談セッション画面の両方から使う。
 */
export function ResearchSessionPanel({ embedded = false }: ResearchSessionPanelProps) {
  ...
  // 埋め込み時、読込中や未受講では何も出さない（通常生徒のセッション画面を汚さない）
  if (loading) {
