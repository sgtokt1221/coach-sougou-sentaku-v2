  ...
  for (const student of students) {
    ...
    if (!student.lastActivityAt) {
      ...
      if (daysSince >= 7) {
        add(student, undefined, {
          ...
          recommendedAction:
            "フォローアップの連絡を入れ、モチベーション向上を図りましょう。",
        });
      }
    }
    ...
    if (student.apAlignmentScores.length >= 5) {
      ...
      if (allBelow) {
        const avg =
          Math.round(
            (recent5.reduce((s, v) => s + v, 0) / recent5.length) * 10
          ) / 10;
        add(student, undefined, {
          type: "ap_struggle",
          ...
          detectedAt: new Date().toISOString(),
          recommendedAction:
            "志望校のアドミッション・ポリシーを一緒に読み直し、出題意図への理解を深めましょう。",
        });
      }
    }
    ...
    if (upcomingDocs.length > 0) {
      const totalWithDeadline = student.documents.filter(
        (d) => d.deadline
      ).length;
      const incompleteCount = upcomingDocs.length;
      const incompleteRate =
        totalWithDeadline > 0 ? incompleteCount / totalWithDeadline : 0;
      ...
      if (incompleteRate >= 0.5 && incompleteCount >= 2) {
        add(student, undefined, {
          ...
          detectedAt: new Date().toISOString(),
          recommendedAction:
            "書類作成の優先順位を生徒と一緒に整理し、集中的に取り組むスケジュールを立てましょう。",
        });
      }
    }
    ...
    if (hasLowVariance(scores, 3)) {
      ...
      const avg =
        Math.round((recent4.reduce((s, v) => s + v, 0) / recent4.length) * 10) /
        10;
      add(student, undefined, {
        type: "score_plateau",
        ...
        detectedAt: new Date().toISOString(),
        recommendedAction:
          "現在の学習方法を見直し、新しいテーマや形式の小論文に挑戦させましょう。",
      });
    }
    ...
    const studentDataList: StudentAlertData[] = await Promise.all(
      studentDocs.map(async (docSnap) => {
        ...
        const essaysSnap = await adminDb!
          ...
          .catch((e) => {
            console.warn(
              `[admin/alerts] essays query failed for ${studentUid}:`,
              e
            );
        ...
        /**
         * 成長停滞（score_plateau）は分散で見る。満点の違う答案（口頭試問型は60点）
         * が混ざると分散が跳ね、停滞していても検知できない。50点スケールへ揃える。
         */
        ...
        const documentsSnap = await adminDb!
          ...
          .catch((e) => {
            console.warn(
              `[admin/alerts] documents query failed for ${studentUid}:`,
              e
            );
            return { docs: [] as FirebaseFirestore.QueryDocumentSnapshot[] };
          });
        ...
        const acksSnap = await adminDb!
          ...
          .catch((e) => {
            console.warn(
              `[admin/alerts] alertAcks query failed for ${studentUid}:`,
              e
            );
            return { docs: [] as FirebaseFirestore.QueryDocumentSnapshot[] };
          });
        ...
        const isRecent = (ms: number) =>
          (nowMs - ms) / (1000 * 60 * 60 * 24) <= NEW_WINDOW_DAYS;
        ...
        try {
          const iSnap = await adminDb!
            ...
            .where("userId", "==", studentUid)
            .get();
          let latestInterview: FirebaseFirestore.QueryDocumentSnapshot | null =
            null;
          let latestMs = -1;
...
export async function POST(request: NextRequest) {
  ...
  if (!studentUid || !rawKey) {
    return NextResponse.json(
      { error: "studentUid と alertKey は必須です" },
      { status: 400 }
    );
  }
