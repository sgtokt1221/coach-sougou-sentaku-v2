...
export async function POST(request: NextRequest) {
  ...
  if (
    ...
    typeof blankIndex !== "number" ||
    !studentText?.trim()
  ) {
    return NextResponse.json(
      { error: "passageId / blankIndex / studentText が必要です" },
      { status: 400 }
    );
  }
  if (studentText.length > 2000) {
    return NextResponse.json(
      { error: "文章が長すぎます（2000字以内）" },
      { status: 400 }
    );
  }
  ...
  const scores = {
    ...evaluation.scores,
    total: computeChocoTotal(evaluation.scores),
  };
  ...
  if (adminDb) {
    ...
    void import("@/lib/skill-check/aggregate")
      ...
      .catch((e) =>
        console.warn("[choco-review] aggregate refresh failed:", e)
      );
    void import("@/lib/bigquery/logger")
      .then(({ logEssaySubmission }) =>
        logEssaySubmission({
          ...
          // AP合致度・回答力・議論の成熟度を採点できないので null にする
