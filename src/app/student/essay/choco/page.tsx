...
import {
  CHOCO_FACULTIES,
  getChocoPassagesByFaculty,
  ALL_CHOCO_PASSAGES,
} from "@/data/choco-passages";
...
export default function ChocoPage() {
  ...
  /**
   * コーチに渡す設問。題名だけだと何を書かせているのか伝わらず、
   * 「どんな問いが出ていますか」と聞き返されてしまう。
   */
  const coachTopic = useMemo(() => {
    if (!passage) return "";
    const role = passage.paragraphs[blankIndex]?.role;
    const roleLabel = role ? CHOCO_ROLE_LABELS[role] : "";
    return [
      passage.themeTitle,
      `全${passage.paragraphs.length}段落の文章のうち、${blankIndex + 1}段落目${roleLabel ? `（${roleLabel}）` : ""}が空欄です。`,
      "前後の段落とつながるように、その段落だけを書きなさい。",
    ].join("\n");
  }, [passage, blankIndex]);
  ...
  const { status, restored, lastSavedAt, saveNow, clearDraft } =
    usePersistentDraft({
      key: "essay-choco",
      value: {
        facultyKey,
      ...
      onRestore: (draft) => {
        const savedPassage = draft.passageId
          ? (ALL_CHOCO_PASSAGES.find((item) => item.id === draft.passageId) ??
            null)
          : null;
  ...
  useEffect(() => {
    ...
    fetch(`/api/universities/resolve?ids=${targetUniversities.join(",")}`)
      ...
      .then((d) => {
        if (active) setResolved(d.resolved ?? []);
      })
      .catch(() => {
    ...
    return (
      <div className="mx-auto max-w-7xl p-4">
        <DraftSaveIndicator
          ...
          className="mb-3"
        />
        <div className="lg:grid lg:grid-cols-[minmax(22rem,28rem)_minmax(0,1fr)] lg:items-start lg:gap-6">
          {/* 左: 本文 ⇄ AIコーチ を切り替え (PC のみ・現状維持) */}
          <div className="lg:sticky lg:top-4">
            <div className="hidden lg:block">
              <div className="bg-muted/40 mb-2 inline-flex rounded-lg border p-1">
                {[
              ...
              {leftView === "passage" ? (
                ...
                <div className="flex h-[70vh] flex-col">
                  {resolved.length > 0 && (
                    <select
                      className="bg-background mb-2 w-full rounded-lg border p-2 text-xs"
                    ...
                    >
                      {resolved.map((r) => (
                        <option
                          ...
                          value={`${r.universityId}:${r.facultyId}`}
                        >
          ...
          <MobileSlideOverPanel label="AIコーチ" title="AIコーチ">
            <div className="flex h-full min-h-0 flex-col">
              {resolved.length > 0 && (
                <select
                  className="bg-background mb-2 w-full shrink-0 rounded-lg border p-2 text-xs"
                  value={selectedCompoundId ?? ""}
                  onChange={(e) => setSelectedCompoundId(e.target.value)}
                >
                  {resolved.map((r) => (
          ...
          {/* 右: 入力 */}
          <div className="mt-4 space-y-3 lg:mt-0 lg:min-w-0">
            <p className="text-muted-foreground text-sm">
              本文の空欄（{passage.paragraphs.length}段落中 {blankIndex + 1}
              段落目）を書いてみよう。 困ったら「AIコーチ」に相談できます。
            </p>
            <ManuscriptEditor
              value={text}
              onChange={setText}
              maxLength={300}
              placeholder="この段落を書いてみよう..."
            />
  ...
  return (
    <div className="mx-auto max-w-2xl p-4 sm:p-6">
      {/* ヒーロー */}
      <div className="relative overflow-hidden rounded-2xl border bg-gradient-to-br from-teal-500 to-emerald-600 p-6 text-white shadow-sm sm:p-8">
        ...
        <div className="pointer-events-none absolute -top-10 -right-10 size-44 rounded-full bg-white/10" />
      ...
      <Button
        onClick={startNew}
        size="lg"
        className="mt-6 w-full gap-2 bg-teal-500 text-white hover:bg-teal-600"
      >
        はじめる
