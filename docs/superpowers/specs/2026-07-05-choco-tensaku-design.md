...
## 4. ユーザー体験（3ステップ）
1. **開始**: **系統（学部分野）を選ぶ**（要約ドリルと同じ `FACULTY_REGISTRY` の系統キー。初期表示は先頭/直近を既定選択。※プロフィールの志望大学→系統の自動マップは既存に無いため、要約ドリル同様の手動選択とする）。選んだ系統の本文を1本ランダム抽選し、「別の本文にする」ボタンで引き直し可。
2. **記入**: **2カラムのside-by-sideレイアウト**。
   - **左パネル＝穴あき本文を常時固定表示（sticky／スクロール追従）**。通常の小論文エディタで「左のAIチャットモーダル」が入る参照枠の位置に、本文全体を配置し、伏せた段落は「【ここを書く：根拠の段落】」の枠として明示。生徒が右で書いている間も常に本文を見ながら書ける（この機能では左にAIチャットは出さず、本文の参照に充てる）。`keyPoints`（背景知識）は伏せる＝挑戦。
   - **右パネル＝入力欄**。欠落段落だけを既存 `ManuscriptEditor`（字数カウント付き、目安150〜250字）で入力。
   - モバイル（1カラム）時は、本文を上に折り畳み可能な形で常時アクセスできるようにし、入力欄を下に置く。
3. **結果**: 点数（3軸＋total）＋**3軸レーダーチャート**（既存 Recharts `RadarChart` を流用）＋講評＋良かった点／もう一歩＋**赤ペン**（`RedPenText`）＋**模範段落＋この段落で押さえたい背景知識**＋次の一手1つ。
...
## 8. UI コンポーネント
- 再利用: `ManuscriptEditor`（入力）、`RedPenText`＋`LanguageCorrection`（赤ペン）、`RadarChart`（Recharts、既存 essay `[id]/page.tsx` の実装を流用）、side-by-side（2カラム）レイアウトと summary-drill の3ステップ構成/スタイル。
- 新規:
  - `src/app/student/essay/choco/page.tsx`（3ステップ制御）
  - **穴あき本文パネル**（本文を段落配列で描画し、`blankIndex` の位置を「ここを書く」枠＋役割ラベルで表示）。記入ステップでは**左の参照枠に sticky 固定**し、右の入力欄と2カラムで並べる（通常エディタで左にAIチャットモーダルが入る位置）。モバイルは折り畳み可能な上部パネル。
  - `ChocoResultView`（点数3軸＋total、**3軸レーダーチャート**、講評、赤ペン、模範段落＋背景知識、次の一手）
- スコア・レーダーは出す（ユーザー決定）。3軸（論理／つながり／表現）をレーダー表示＋total を併記。
...
## 11. テスト（検証観点）
リポジトリにユニットテスト基盤（jest/vitest）は無く、既存は `tsx` スクリプト（例 `validate-university-data.ts`）＋ Playwright。ちょこ添削も同方針：
- **純粋ロジックは `tsx` 検証スクリプトで assert**：
  - total 換算 `round((logic+coherence+expression)/30*50)` の境界値。
  - 重み付き平均 `blendPracticeScores`（essayのみ／chocoのみ／混在／空）。
- **データ検証スクリプト**：各 ChocoPassage が段落4〜5・role 妥当・keyPoints 非空・本文800字前後。
- **`npm run build`（型・ビルド）＋ dev 目視**：穴埋めUIが `blankIndex` 位置に入力欄／結果のレーダー・赤ペン・模範表示／保存が `chokoReviews` に入る／弱点DB加算／BQ mock ログに `essay_type:"choco"`／ランクが choco 経由でも付与。

## 12. 初期バンク（人手で用意、たたき台）
**教育・看護医療・経済経営・人文社会・理工・薬学・現代社会学 の7分野 × 各2本（計14本）**。各本文800字前後・4〜5段落・各段落に role と keyPoints（背景知識）。分野・本数はレビューで調整。
